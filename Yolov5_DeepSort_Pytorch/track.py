            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        persons.append({'id': id, 'coords': bboxes, 'cutout': im0[output[0]:output[2], output[1]:output[3]]})

                        c = int(cls)  # integer class
                        label = process_inference.cachedNames[id] if id in process_inference.cachedNames else 'Loading..'
                ...
                process_inference.process(persons)
